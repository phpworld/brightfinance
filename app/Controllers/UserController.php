    ...
    if ($this->request->getMethod() == 'POST') {
        ...
        if (!$this->validate($rules)) {
            ...
            if ($user) {
                ...
                if ($user['status'] == 0) {
                    return redirect()->to('admin/login')->with('error', 'Your account is inactive. Please contact support.');
                }
    ...
    if ($this->request->getMethod() == 'POST') {
        ...
        if (!$this->validate($rules)) {
            ...
            if (!password_verify($this->request->getVar('old_password'), $user['password'])) {
                return redirect()->to('admin/change-password')->with('error', 'Old password is incorrect.');
            }
